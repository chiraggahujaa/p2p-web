...
import { User, Camera, Upload, AlertCircle, Trash2, BadgeCheck, Save, X, Move } from 'lucide-react';
...
interface AvatarEditorProps {
  ...
  onEditStart?: () => void;
  onEditCancel?: () => void;
  onEditSave?: (avatarUrl: string | null) => void;
  onUploadStart?: () => void;
  onUploadComplete?: (avatarUrl: string) => void;
  onUploadError?: (error: string) => void;
  disabled?: boolean;
  className?: string;
  showUploadButton?: boolean; // Control upload button visibility
}
...
export function AvatarEditor({
  ...
  const createCroppedImage = async (imageSrc: string, position: ImagePosition): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const scaledWidth = img.width * position.scale;
        const scaledHeight = img.height * position.scale;
        
        // Draw the image with position and scale
        ctx.drawImage(
          img,
          position.x - scaledWidth / 2 + size / 2,
          position.y - scaledHeight / 2 + size / 2,
          scaledWidth,
          scaledHeight
        );
  ...
  const handleFiles = useCallback(async (acceptedFiles: File[]) => {
    ...
    if (file.size > maxSizeBytes) {
      const errorMsg = `File size exceeds ${Math.round(maxSizeBytes / 1024 / 1024)}MB limit.`;
      setError(errorMsg);
      onUploadError?.(errorMsg);
      return;
    }
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      const croppedImageUrl = await createCroppedImage(tempImageUrl, imagePosition);
            ...
            <div className="relative">
              ...
              >
                ...
                <Avatar className={cn(
                  ...
                  isDragActive ? 'border-primary border-dashed' : 'border-muted',
